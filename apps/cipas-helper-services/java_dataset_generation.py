...
DEFAULT_MAX_TOKENS = 1024 # Reduced from 2048 for speed
DEFAULT_CLONES_PER_TYPE = 100 # Target number of clones per type GLOBAL
DEFAULT_MAX_RETRIES = 1 # Maximum number of repair attempts (Reduced from 3 for speed)
...
1. Perform EXACTLY 2 or 3 of the following modifications (do NOT over-modify):
...
def generate_with_repair(model_manager: ModelManager, prompt, problem_id, codenet_df, max_retries=DEFAULT_MAX_RETRIES, max_new_tokens=DEFAULT_MAX_TOKENS):
    temperatures = [0.1, 0.3, 0.5]
    
    for attempt in range(max_retries):
        temp = temperatures[min(attempt, len(temperatures) - 1)]
        raw, error = model_manager.generate(prompt, max_tokens=max_new_tokens, temperature=temp)
        ...
        current_code = code
        for repair_attempt in range(1): # Reduced from 2 to 1 for speed
            repair_prompt = REPAIR_PROMPT_TEMPLATE.replace("<<<CODE_PLACEHOLDER>>>", current_code).replace("<<<ERROR_PLACEHOLDER>>>", str(error_msg))
            raw_repair, err = model_manager.generate(repair_prompt, max_tokens=max_new_tokens, temperature=0.1)
...
def generate_clone(model_manager, code, clone_type, problem_id, codenet_df):
    ...
    # Custom token limit for Type 4
    current_max_tokens = 5000 if clone_type == 'type4' else DEFAULT_MAX_TOKENS

    if clone_type == 'type3':
        for attempt in range(2): # Reduced from 5 to 2 for speed
            generated_code = generate_with_repair(model_manager, prompt, problem_id, codenet_df, max_new_tokens=current_max_tokens)
        ...
        return generate_with_repair(model_manager, prompt, problem_id, codenet_df, max_new_tokens=current_max_tokens)
...
def main():
    ...
    parser.add_argument("--target", type=int, default=DEFAULT_CLONES_PER_TYPE, help="Target clones per type")
            ...
            with jsonlines.open(clone_output_file) as reader:
                for obj in reader:
                    ...
                    if ctype in target_types:
                        type_counts[ctype] += 1
            print(f"Current counts: {type_counts}")
        except Exception as e:
            print(f"{Fore.RED}Error reading existing file: {e}. Will append knowing risks.{Style.RESET_ALL}")

    # Check if we are already done
    def all_complete():
        return all(c >= args.target for c in type_counts.values())

    if all_complete():
        print(f"{Fore.GREEN}All clone types have reached the target ({args.target} each). Exiting.{Style.RESET_ALL}")
        return
    ...
    print(f"Processing problems to find {args.target} samples per type...")
    # Calculate remaining work for progress bar
    total_needed = len(target_types) * args.target
    ...
    # Progress bar for TOTAL CLONES
    pbar = tqdm(total=total_needed, initial=current_total, desc="Total Clones", unit="clone")
    
    for problem_id in problems:
        # STOP CONDITION: All types generated to target
        if all_complete():
            print(f"\n{Fore.GREEN}Successfully generated {args.target} clones for ALL types. Stopping.{Style.RESET_ALL}")
            break
        ...
        # OPTIMIZATION: Skipping seed validation to speed up process. We trust 'accepted' status from Parquet.
        # if validate_java(seed_code, problem_id, codenet_df) != "passed":
        #    continue
            
        # Attempt to generate MISSING types
        for gen_type in target_types:
            if type_counts[gen_type] >= args.target:
                continue
            
            print(f"Attempting {gen_type} ({type_counts[gen_type]}/{args.target}) using problem {problem_id}...")
            ...
            try:
                ...
                if generated:
                    ...
                    stats['success'] += 1
                    pbar.update(1)
                    print(f"  {Fore.GREEN}✓ {gen_type} GENERATED & SAVED. (Total: {type_counts[gen_type]}/{args.target}){Style.RESET_ALL}")
                else:
                    stats['failed'] += 1
    ...
    if not all_complete():
        print(f"\n{Fore.YELLOW}Finished processing problems but did NOT complete all targets.{Style.RESET_ALL}")
        print(f"Current Counts: {type_counts}")
    else:
