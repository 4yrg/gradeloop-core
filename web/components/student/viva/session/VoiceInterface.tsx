...
export function VoiceInterface({ isAiSpeaking, isUserSpeaking, transcription }: VoiceInterfaceProps) {
    // Fake waveform bars
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="flex items-end gap-1.5 h-32">
                    {bars.map((_, i) => (
                        <div
                            key={i}
                            className={`w-3 rounded-full bg-primary transition-all duration-100 ease-in-out ${isAiSpeaking ? 'animate-waveform' : 'h-2 opacity-20'}`}
                            style={{
                                animationDelay: `${i * 0.05}s`,
                                height: isAiSpeaking ? `${Math.max(20, Math.random() * 100)}%` : '8px'
                            }}
                        />
                    ))}
                </div>
            ...
            <div className="relative p-6 bg-gradient-to-t from-black via-black/80 to-transparent">
                <div className={`flex items-end gap-4 transition-all duration-300 ${isUserSpeaking ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-50'}`}>
                    <div className="h-10 w-10 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center shrink-0">
                        <Mic className={`h-5 w-5 ${isUserSpeaking ? 'text-green-500' : 'text-zinc-500'}`} />
                    </div>
                    <div className="flex-1">
                        <div className="text-xs text-zinc-400 mb-1 uppercase tracking-wider font-semibold">Live Transcription</div>
                        <p className="text-lg text-white font-medium leading-relaxed">
                            {transcription || <span className="text-zinc-600 italic">Listening for your answer...</span>}
                        </p>
                    </div>
                </div>
            </div>
